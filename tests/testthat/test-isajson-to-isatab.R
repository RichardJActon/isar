test_that("isa-json can be converted to isa-tab", {
	expect_true(exists("Investigation"))
	# Can isar generate:
	# - i_investigation.txt
	# - a_metabolome.txt
	# - a_microarray.txt
	...
	# Assays ----
	test_that("Assay tables can be generated from isa-json", {
		expect_true(exists("Assay"))
		## a_proteome.txt ----
		...
		# - There appears to be an issue with representing the relationship between Materials and DataFiles when values of these objects relate to particular StudyFactor values.
		# - As we can't assume all combinations of FactorValues, Materials, & DataFiles exist a way is needed to represent these relationships.
		# - The a_proteome.txt example from BII-I-1 for example cannot as far as I can tell be correctly reconstructed from the BII-I-1.json representation.
		# - The StudyFactor 'limiting nutrient'
		# 	- has 5 possible values:
		# 		- Sulphur
		# 		- Carbon
		# 		- Nitrogen
		# 		- Phosphorus
		# 		- NA (not explicitly represented)
		# 	- NA likely need to be explicitly represented in the JSON serialisation in order for these relationships to be specified
		# 	- 'Labelled Extract Name' Contains pooled extracts which may have a combination of limiting nutrients
		# - For example: The sample P-0.2-aliquot11 can have the Labeled Extract Name of Pool3 or JC_P-0.2, if the former the limiting nutrient is NA and the label is either iTRAQ reagent 117 or 114 and if the latter then the limiting nutrient is Phosphorus and the label is iTRAQ reagent 116.
		# - There are 8 samples, 11 'labelled extract names', 5 different 'limiting nutrients', and 4 different labels
		# - Without the ability to create a relationship between a specific study factor value and a Material or Data File this table cannot be reconstructed with the correct subset of possible relationships between these values.
		#
